"""
Script para extraer iconos individuales de iconsassets.png
Separa cada icono y elimina TODO el espacio no usado (recorte exacto).
"""
...
def extract_icons_tight(input_path, output_dir, icon_names, cols=2, rows=2, padding=2):
    """
    Extrae iconos individuales de una imagen en cuadrícula con recorte exacto.
    
    Args:
        input_path: Ruta de la imagen de entrada
        output_dir: Directorio de salida
        icon_names: Lista de nombres para cada icono (orden: izq-der, arriba-abajo)
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if bbox:
                ...
                print(f"   ⏭️  Icono {icon_index + 1}: vacío, omitiendo")
...
def main():
    print("=" * 70)
    print("🏗️  EXTRACTOR DE ICONOS - RECORTE EXACTO")
    print("=" * 70)
    
    # Definir la imagen y los nombres de los iconos
    input_path = 'public/assets/textures/iconsassets.png'
    output_dir = 'extracted_icons'
    
    # Nombres de los iconos en orden (izquierda a derecha, arriba a abajo)
    icon_names = [
        'warehouse',   # Fila 1, Columna 1
        'bonfire',     # Fila 1, Columna 2
        'house',       # Fila 2, Columna 1
        'barn'         # Fila 2, Columna 2
    ]
    
    try:
        extracted = extract_icons_tight(
            input_path,
            output_dir,
            icon_names,
            cols=2,
            rows=2,
            padding=2  # Padding mínimo de 2px
        )
